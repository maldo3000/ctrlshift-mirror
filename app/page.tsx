...
export default function Home() {
  ...
  const stateRef = useRef<EffectSettings>({mode:3,intensity:.72,feedback:.68,pixel:.08,threshold:.28,particles:.82,particlesEnabled:1,mirror:1,frozen:false,hasVideo:false});
  const [preset,setPreset]=useState(3),[intensity,setIntensity]=useState(72),[feedback,setFeedback]=useState(68);
  const [pixel,setPixel]=useState(8),[threshold,setThreshold]=useState(28),[particles,setParticles]=useState(82);
  const [particleEnabled,setParticleEnabled]=useState([true,true,true,true,true,true,false,false]);
  ...
  useEffect(()=>{
    ...
    try { renderer=new VisualRenderer(canvasRef.current!,setRenderError,()=>setPreset(v=>(v+1)%PRESETS.length));rendererRef.current=renderer; }
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.repeat||e.ctrlKey||e.metaKey||e.altKey||(e.target instanceof HTMLElement&&e.target.closest("input,textarea,select,button,[contenteditable=true],[role=slider],[role=switch]")))return;if(e.key.toLowerCase()==="f")toggleProjection();if(e.key.toLowerCase()==="p")setFrozen(v=>!v);if(e.code==="Space"){e.preventDefault();setPreset(v=>(v+1)%PRESETS.length);}if(/^[1-8]$/.test(e.key))setPreset(Number(e.key)-1);};addEventListener("keydown",key);return()=>removeEventListener("keydown",key);},[toggleProjection]);
  useEffect(()=>{
    ...
    void Promise.resolve(context.registerTool({
      ...
      description:"Select one of the eight visual presets and optionally tune its effect values.",
      inputSchema:{type:"object",properties:{preset:{type:"integer",minimum:1,maximum:8},intensity:{type:"number",minimum:0,maximum:100},echo:{type:"number",minimum:0,maximum:100},pixelate:{type:"number",minimum:0,maximum:100},threshold:{type:"number",minimum:0,maximum:100},particles:{type:"number",minimum:0,maximum:100},particlesEnabled:{type:"boolean"}},additionalProperties:false},
      annotations:{readOnlyHint:false,untrustedContentHint:false},
      execute(input:unknown){
        if(!input||typeof input!=="object")throw new Error("Input must be an object.");
        const value=input as ToolInput;
        if(value.preset!==undefined&&(!Number.isInteger(value.preset)||value.preset<1||value.preset>8))throw new Error("Preset must be 1–8.");
  ...
  return <main className={projectionMode?"app projection":"app"} style={{"--accent":PRESETS[preset].accent} as React.CSSProperties}>
    ...
    <section className="workspace">
      <aside className="presets chrome" aria-label="Effect presets"><div className="panel-title"><Aperture size={16}/><span>PATCH SELECT</span></div><div className="preset-list">{PRESETS.map((p,i)=><button key={p.code} onClick={()=>setPreset(i)} className={preset===i?"preset active":"preset"}><span>{p.code}</span><b>{p.name}</b><em>{String(i+1).padStart(2,"0")}</em></button>)}</div><div className="signal-chain"><small>SIGNAL PATH</small><div><span>CAM</span><i/><span>GLSL</span><i/><span>OUT</span></div></div></aside>
